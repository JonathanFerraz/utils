...
function parsePhoneDigits(phone: string): {
...
export function isValidMobilePhone(phone: string): boolean {
  ...
  return (
    isValidMobilePhoneLength(digits) &&
    isValidMobilePhoneFirstNumber(digits) &&
    isValidDDD(digits)
  );
}
...
export function isValidLandlinePhone(phone: string): boolean {
  ...
  return (
    isValidLandlinePhoneLength(digits) &&
    isValidLandlinePhoneFirstNumber(digits) &&
    isValidDDD(digits)
  );
}
...
export function isValid(phone: string): boolean {
  ...
  return (
    isValidLength(digits) && isValidFirstNumber(digits) && isValidDDD(digits)
  );
}
