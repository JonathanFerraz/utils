...
export function generate(): string {
  ...
  const firstCheckDigitMod =
    generateChecksum(baseCNPJ, FIRST_CHECK_DIGIT_WEIGHTS) % 11;
  const firstCheckDigit = (
    firstCheckDigitMod < 2 ? 0 : 11 - firstCheckDigitMod
  ).toString();

  const secondCheckDigitMod =
    generateChecksum(baseCNPJ + firstCheckDigit, SECOND_CHECK_DIGIT_WEIGHTS) %
    11;
...
export function isValid(cnpj: string): boolean {
  ...
  return (
    isValidFormat(cnpj) &&
    !isReservedNumber(numbers) &&
    isValidChecksum(numbers)
  );
}
